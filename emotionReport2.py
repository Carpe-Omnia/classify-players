...
def generate_emotion_chart_base64(df):
    ...
    ax.tick_params(axis='x', rotation=45, colors='#F0F8FF') # Rotate x-axis labels, off-white
    ax.tick_params(axis='y', colors='#F0F8FF') # off-white y-axis ticks

    # Set background for the plot area
    ax.set_facecolor('#20452E') # Darker green for plot area
    fig.patch.set_facecolor('#20452E') # Match figure background

    # Add value labels on top of bars
    for bar in bars:
        yval = bar.get_height()
        ax.text(bar.get_x() + bar.get_width()/2, yval + 0.5, int(yval), ha='center', va='bottom', fontsize=10, color='#F0F8FF') # Off-white text

    ax.grid(axis='y', linestyle='--', alpha=0.7, color='#66BB6A') # Lighter green grid lines
    plt.tight_layout()
...
def generate_player_emotions_report(input_csv_path="combined_depth_charts/processed_player_data.csv",
    ...
    try:
        ...
        df_merged = pd.merge(
            df_processed, 
            df_master[['PlayerUID', 'TeamName', 'PrimaryPosition']].drop_duplicates(subset=['PlayerUID']), 
            on='PlayerUID', 
            how='left'
        )
