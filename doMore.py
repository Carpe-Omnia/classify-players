...
if __name__ == "__main__":
            ...
            with open(output_results_csv, 'r', encoding='utf-8', newline='') as existing_outfile:
                ...
                for row in reader:
                    ...
                    if player_uid: # Ensure PlayerUID exists
                        ...
                        inferred_age_status = row.get('InferredAge', 'N/A')
                        inferred_emotion_status = row.get('InferredEmotion', 'N/A')

                        # Check if ALL three relevant fields indicate successful processing
                        # A field is considered 'successful' if it's not in failed_statuses and doesn't start with 'Error:'
                        ...
                        is_age_successful = (inferred_age_status != 'N/A' and 
                                             not str(inferred_age_status).startswith('Error:') and
                        ...
                        # Only add to processed_uids if ALL requested analysis types are successful
                        if is_race_successful and is_age_successful and is_emotion_successful:
    ...
    try:
        ...
        fieldnames = ['PlayerName', 'PlayerUID', 'InferredRace', 'RaceConfidence', 
                      'InferredAge', 'InferredEmotion', 'EmotionConfidence', 'PlayerURL']
            ...
            for i, row in enumerate(reader):
                ...
                try:
                    ...
                    if not player_url or player_url == 'Empty Slot':
                        ...
                        player_analysis_data['InferredAge'] = 'N/A (No URL)'
                        player_analysis_data['InferredEmotion'] = 'N/A (No URL)'
                        player_analysis_data['EmotionConfidence'] = '0.00%'
                        ...
                        if image_link:
                            ...
                            try:
                                ...
                                if data_size > 0:
                                    ...
                                    try:
                                        # Modified actions to include 'age' and 'emotion'
                                        demography = DeepFace.analyze(
                                            img_path=image_filepath,
                                            actions=['age', 'emotion', 'race'], # Added 'age' and 'emotion'
                                        ...
                                        if demography:
                                            ...
                                            player_analysis_data['RaceConfidence'] = 'N/A'
                                            player_analysis_data['InferredAge'] = 'N/A (No Face Detected)'
                                            player_analysis_data['InferredEmotion'] = 'N/A (No Face Detected)'
                                            player_analysis_data['EmotionConfidence'] = 'N/A'
                                        ...
                                        player_analysis_data['RaceConfidence'] = 'N/A'
                                        player_analysis_data['InferredAge'] = f'Error: {str(e)[:50]}...'
                                        player_analysis_data['InferredEmotion'] = f'Error: {str(e)[:50]}...'
                                        player_analysis_data['EmotionConfidence'] = 'N/A'
                                    ...
                                    player_analysis_data['InferredAge'] = 'N/A (Empty Download)'
                                    player_analysis_data['InferredEmotion'] = 'N/A (Empty Download)'
                                    player_analysis_data['EmotionConfidence'] = '0.00%'
                                ...
                                player_analysis_data['RaceConfidence'] = '0.00%'
                                player_analysis_data['InferredAge'] = f'N/A (Download Error: {str(e)[:50]}...)'
                                player_analysis_data['InferredEmotion'] = f'N/A (Download Error: {str(e)[:50]}...)'
                                player_analysis_data['EmotionConfidence'] = '0.00%'
                        else:
                            print(f"  [Scraper] No valid image URL retrieved for {player_name}. Skipping DeepFace analysis.")
                            player_analysis_data['InferredRace'] = 'N/A (Scrape Failed)' 
                            player_analysis_data['RaceConfidence'] = '0.00%'
                            player_analysis_data['InferredAge'] = 'N/A (Scrape Failed)'
                            player_analysis_data['InferredEmotion'] = 'N/A (Scrape Failed)'
                            player_analysis_data['EmotionConfidence'] = '0.00%'
                    ...
                    player_analysis_data['RaceConfidence'] = 'N/A'
                    player_analysis_data['InferredAge'] = f'CRITICAL ERROR: {str(e)[:50]}...'
                    player_analysis_data['InferredEmotion'] = f'CRITICAL ERROR: {str(e)[:50]}...'
                    player_analysis_data['EmotionConfidence'] = 'N/A'
